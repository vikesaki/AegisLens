...
# def app():
#     # st.set_page_config(page_title="Video Prediction", layout="centered")

#     st.title("Video Prediction with YOLOv8")
#     st.write("""
#     Upload a .mov or .mp4 video (max 200MB) to run inference using the trained model (35epoch.pt).
#     """)

#     uploaded_file = st.file_uploader("Choose a video file (.mov, .mp4)", type=["mp4", "mov"])
...
#             # Run prediction
#             results = model.predict(tfile.name, save=True, conf=0.25)

#             # Find the output video path
#             output_dir = results[0].save_dir
#             output_files = [f for f in os.listdir(output_dir) if f.endswith(('.mp4', '.mov'))]
#             if output_files:
